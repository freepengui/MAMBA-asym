...
public:
  ...
  void append1(const BitVector &K)
  {
    size_t totalBits = nbits + K.nbits;
    if (nbits != totalBits)
      {
        resize(totalBits);
      }
    memcpy(bytes+nbytes-1, K.bytes, nbytes);
  }

  void append2(const BitVector &K)
  {
    size_t totalBits = nbits + K.nbits;
    if (nbits != totalBits)
      {
    ...
    {
      set_byte(nbytes+i, K.get_byte(i));
    }
  }

  BitVector &combine(const BitVector &K)
  {
    size_t totalBits = nbits + K.nbits;
    BitVector newBitVector(totalBits);
    for (size_t i = 0; i < nbytes; i++)
    {
      newBitVector.set_byte(i, get_byte(i));
      newBitVector.set_byte(nbytes+i, K.get_byte(i));
    }
    return newBitVector;
  }
  ...
  {
    unsigned long int j= i / 8, k= i & 7;  // 取余
    if (a == 1)
      {
