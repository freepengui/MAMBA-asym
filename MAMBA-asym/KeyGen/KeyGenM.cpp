...
KeyGenM::KeyGenM(Player &Pl) : P(Pl) // 初始化列表的构造函数
{
}
...
void KeyGenM::genKey(unsigned int N, bigint fsize1, bigint fsize2, bigint pMPC, unsigned int nb_threads_1, unsigned int nb_threads_2, SSL_CTX *ctx, const SystemData &SD, vector<vector<vector<int>>> &csockets, vector<gfp> &MacK)
{
  ...
  // cout << "mt1.Multiply" << endl;
  mt1.Multiply();
  // cout << "mt1.Combine" << endl;
  mt1.Combine();
  // cout << "mt1.Authenticate" << endl;
  mt1.Authenticate();
  // cout << "mt1.Sacrifice()" << endl;
  mt1.Sacrifice();

  // cout << "mt1" << endl;
  endTime= clock();
  cout << "The running time of triples generation is: " << (double) (endTime - startTime) / CLOCKS_PER_SEC << "s" << endl;

  // Init the factories
  gfp::init_field(fsize2);
  // Player Pmt2(whoami, SD, 0, ctx, csockets[nbThreads1 + 2], MacK, 0);
  MASCOTTriples mt2(P, fsize2);
  // cout << "mt2.Multiply" << endl;
  mt2.Multiply();
  // cout << "mt2.Combine" << endl;
  mt2.Combine();
  // cout << "mt2.Authenticate" << endl;
  mt2.Authenticate();
  // cout << "mt2.Sacrifice()" << endl;
  mt2.Sacrifice();

  sec= 40;
  bit_batch_size= 51000;

  cout << "mt1 Delta " << mt1.Delta << endl;
  cout << "mt2 Delta " << mt2.Delta << endl;
  ...
  // Init mutexes
  //  cout << "nbThreas1= " << nbThreads1 << "   --- nbThreads2= " << nbThreads2 << endl;
  mtx1.resize(nbThreads1); // queue<mutex>
  ...
  for (unsigned int i= 1; i < whoami; i++)
    {
      a_Input_other(i, sk_w1[i], mac_sk_w1[i], fsize1, mt1);
    }

  if (whoami != 0)
    {
      a_Input_self(my_sk_w1, sk_w1[whoami], mac_sk_w1[whoami], fsize1, mt1);
    }
  ...
  for (unsigned int i= 1; i < whoami; i++)
    {
      a_Input_other(i, sk_w2[i], mac_sk_w2[i], fsize2, mt2);
    }

  if (whoami != 0)
    {
      a_Input_self(my_sk_w2, sk_w2[whoami], mac_sk_w2[whoami], fsize2, mt2);
    }
  ...
  if (whoami == 0)
    {
      ...
      a_Input_self(my_epsilon_w1, epsilon_w1, epsilon_mac_w1, fsize1, mt1);
    }
  else
    {
      a_Input_other(0, epsilon_w1, epsilon_mac_w1, fsize1, mt1);
    }
  ...
  if (whoami == 0)
    {
      ...
      a_Input_self(my_epsilon_w2, epsilon_w2, epsilon_mac_w2, fsize2, mt2);
    }
  else
    {
      a_Input_other(0, epsilon_w2, epsilon_mac_w2, fsize2, mt2);
    }
...
void KeyGenM::RandomMaBit(int M, int sec, MASCOTTriples &mt1, MASCOTTriples &mt2, bigint fsize1, bigint fsize2)
{
  ...
  for (unsigned int i= 0; i < whoami; i++)
    {
      a_Input_other(i, b_shared_w2[i], mac_b_shared_w2[i], fsize2, mt2);
    }

  a_Input_self(b_mine_w2, b_shared_w2[whoami], mac_b_shared_w2[whoami], fsize2, mt2);
  ...
  if (P.whoami() != 0)
    ...
    cout << "RandomaBit finished in world 1" << endl;
  ...
  a_Input_self(bits_w2, b_shared_w2[whoami], mac_b_shared_w2[whoami], fsize2, mt2);
  ...
  for (unsigned int i= 0; i < whoami; i++)
    {
      a_Input_other(i, b_shared_w1[i], mac_b_shared_w1[i], fsize1, mt1);
    }

  a_Input_self(bits_w1, b_shared_w1[whoami], mac_b_shared_w1[whoami], fsize1, mt1);
...
void KeyGenM::Uniform(unsigned int N, MASCOTTriples &mt1, MASCOTTriples &mt2, bigint fsize1, bigint fsize2)
{
  ...
  for (unsigned int i= 0; i < whoami; i++) // i < whoami
    {
      a_Input_other(i, v_shared_w1[i], mac_v_shared_w1[i], fsize1, mt1);
    }

  a_Input_self(v_mine_w1, v_shared_w1[whoami], mac_v_shared_w1[whoami], fsize1, mt1); // i = whoami

  for (unsigned int i= whoami + 1; i < nbPlayers; i++) // whoami <= i <= nbPlayers
    {
      a_Input_other(i, v_shared_w1[i], mac_v_shared_w1[i], fsize1, mt1);
    }
  ...
  for (unsigned int i= 0; i < whoami; i++)
    {
      a_Input_other(i, v_shared_w2[i], mac_v_shared_w2[i], fsize2, mt2);
    }

  a_Input_self(v_mine_w2, v_shared_w2[whoami], mac_v_shared_w2[whoami], fsize2, mt2);
 ...
 * 非对称两方
 */
void KeyGenM::a_Input_self(vector<gfp> &in, vector<gfp> &out, vector<gfp> &mac_out, bigint p, MASCOTTriples &mt)
{
  unsigned int nbElements= in.size();
  unsigned int nbPlayers= P.nplayers();
  unsigned int whoami= P.whoami();
  ...
  // 2 - Create random additive sharing for x0 and in
  vector<vector<gfp>> additive_sharing;    //
  additive_sharing.resize(nbElements + 1); // x0
  ...
  for (unsigned int i= 0; i < nbElements + 1; i++)
    {
      ...
      for (unsigned int j= 0; j < nbPlayers - 1; j++)
        {
          additive_sharing[i][j].randomize(G);
          additive_sharing[i][nbPlayers - 1]-= additive_sharing[i][j]; // i=0, x0 - r1 -r2 -...-rn-1; i>0, in - r1 -r2 -rn-1
        }
    }

  for (unsigned int i= 0; i < nbElements; i++)
    {
      out[i]= additive_sharing[i + 1][whoami]; // Player_whoami's share out[i] for in[i]
    }
  ...
  // 3 - Call to COPE.extend as sender
  vector<gfp> x_for_COPE(nbElements + 1); // COPE 输入 xo 和 in[nbElements]
  x_for_COPE[0]= x0;
  for (unsigned int i= 0; i < nbElements; i++)
    {
      x_for_COPE[i + 1]= in[i];
  ...
  gfp mac_x0;
  // 当whoami == 0时，不需要 COPE.extend, 当whoami == 1时，才需要
  if (whoami == 0)
    {
      mac_x0= x0 * mt.Delta; // 好像诚实方不需要 x0，但是为了匹配后续接口   对x0 的mac 没有分享出去
      ostringstream os1;
      gfp complet_mac_shares;
      gfp mac_share_receiver;
      for (unsigned int i= 0; i < nbElements; i++)
        {
          complet_mac_shares= in[i] * mt.Delta;
          mac_out[i].randomize(G); //
          mac_share_receiver = complet_mac_shares - mac_out[i];
          mac_share_receiver.output(os1, false);
        }
      P.send_to_player(1, os1.str(), 1);
    }
  else
    {
      vector<gfp> t;                  // 接收 COPE.Extend 输出
      t.resize(nbElements+1);
      mt.COPE_S[0].extend_vec(P, x_for_COPE, t, true); //
      // 5 - Compute the MAC shares
      for (unsigned int i= 0; i < nbElements + 1; i++)
        {
          if (i == 0)
...
void KeyGenM::Input_other(int nbP_Input, vector<gfp> &out, vector<gfp> &mac_out, bigint p, MASCOTTriples &mt)
{
  ...
  cout << "input other receive from player" << endl;
  gfp share_x0;
  string ss;
  ...
  gfp mac_x0;
  cout << " main thread COPE.extend connection 0" << endl;
 ...
 *
 */
void KeyGenM::a_Input_other(int nbP_Input, vector<gfp> &out, vector<gfp> &mac_out, bigint p, MASCOTTriples &mt)
{
  unsigned int nbElements= out.size();
  unsigned int nbPlayers= P.nplayers();
  unsigned int whoami= P.whoami();
  ...
  for (unsigned int i= 0; i < nbElements + 1; i++)
    {
      if (i == 0)
        {
          share_x0.input(is, false); // x0
        }
      else
        {
          out[i - 1].input(is, false); // for in out[0] ~ out[nbElements-1]
        }
    }
  ...
  if (whoami == 0)
    {
      vector<gfp> receive_COPE(nbElements + 1);
      mt.COPE_R[1].extend_vec(P, receive_COPE, true);
      for (unsigned int i= 0; i < nbElements + 1; i++)
        {
          if (i == 0)
            {
  ...
  else
    {
      string ssn;
      gfp tmp_gfp;
      P.receive_from_player(0, ssn, 1);
      istringstream is2(ssn);
      for (unsigned int i= 0; i < nbElements; i++)
        {
          tmp_gfp.input(is2, false);
          mac_out[i]= tmp_gfp; //
        }
    }
}
